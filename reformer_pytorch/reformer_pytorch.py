...
def process_inputs_chunk(fn, chunks=1, dim=0):
    def inner_fn(*args, **kwargs):
        ...
        chunked_args = list(
            zip(*map(lambda x: x.chunk(chunks, dim=dim),
...
class LSHAttention(nn.Module):
    ...
    def forward(self,
        ...
        assert seqlen % (self.bucket_size * 2) == 0, (
            f'Sequence length ({seqlen}) needs to be divisible by target '
            f'bucket size  x 2 - {self.bucket_size * 2}')
...
class FullQKAttention(nn.Module):
    ...
    def forward(self,
        ...
        if input_attn_mask is not None:
            input_attn_mask = F.pad(
                input_attn_mask, (0, seq_len - input_attn_mask.shape[-1]),
                value=True)
...
class LSHSelfAttention(nn.Module):
    def __init__(self,
        ...
        assert dim_head or (
            dim %
            heads) == 0, 'dimensions must be divisible by number of heads'
    ...
    def forward(self,
        ...
        b, t, e, h, dh, m, l_h = (*x.shape, self.heads, self.dim_head,
                                  self.num_mem_kv, self.n_local_attn_heads)

        mem_kv = default(self.mem_kv,
                         torch.empty(b, 0, e, dtype=dtype, device=device))
...
class Reformer(nn.Module):
    def __init__(self,
        ...
        if weight_tie:
            get_attn, get_ff, get_pkm = map(cache_fn,
                                            (get_attn, get_ff, get_pkm))
...
class ReformerLM(nn.Module):
    def __init__(self,
        ...
        self.to_model_dim = Identity() if emb_dim == dim else nn.Linear(
            emb_dim, dim)
        ...
        if absolute_position_emb:
            ...
            axial_position_shape = default(
                ...
                (math.ceil(max_seq_len / bucket_size), bucket_size))
        ...
        self.out = nn.Sequential(
            ...
            if not weight_tie_embedding else MatrixMultiply(
                self.token_emb.weight, transpose=True, normalize=True))
